...
app.use(
  session({
    ...
    cookie: {
      secure: false, // local дээр https биш тул false
      httpOnly: true,
      sameSite: "lax", // эсвэл "none" (https дээр)
    },
  })
);
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Хэрвээ өмнө нь session-д байгаа бол шууд redirect хийж болно
    if (req.session.user && req.session.user.userId === user.userId) {
...
app.post("/logout", (req, res) => {
  req.session.destroy((err) => {
    ...
    res.clearCookie("connect.sid"); // Session cookie-г устгана
...
app.post("/api/placeadd", async (req, res) => {
  ...
  // Session-аас хэрэглэгчийн ID-г авах
  const createdUserID = req.session?.user?.userId;
  ...
  try {
    ...
    // Session-аас хэрэглэгчийн мэдээллийг авах
